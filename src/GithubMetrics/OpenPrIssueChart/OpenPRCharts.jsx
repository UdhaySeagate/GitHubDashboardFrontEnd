...
/**
 * Following JSX to handle open pr chart
 */
const OpenPRChart = ({ chartData, typeView, charType,dateFileName }) => {
  ...
  const download = () => {
    const nodeList = document.getElementById('barchart').querySelector('svg').querySelectorAll('.c3-chart .c3-chart-line path');
    const nodeList2 = document.getElementById('barchart').querySelector('svg').querySelectorAll('.c3-axis path');
    const nodeList3 = document.getElementById('barchart').querySelector('svg').querySelectorAll('.c3 .tick line');
    const nodeList4 = document.getElementById('barchart').querySelector('svg').querySelectorAll('.c3-legend-item text');
    const nodeList5 = document.getElementById('barchart').querySelector('svg').querySelectorAll('.c3-texts > .c3-text');
    const nodeList6 = document.getElementById('barchart').querySelector('svg').querySelectorAll('.c3-texts > .c3-text.remove');
    ...
    /* eslint-disable no-param-reassign */
    document.getElementById('barchart').querySelector('svg').style.backgroundColor = '#fff';
    document.getElementById('barchart').querySelector('svg').style.font = '10px sans-serif';
    if (document.getElementById('barchart').querySelector('svg .c3-legend-item text')) {
      document.getElementById('barchart').querySelector('svg .c3-legend-item text').style.font = '10px sans-serif';
      document.getElementById('barchart').querySelector('svg .c3-axis-x .c3-axis-x-label').style.transform = 'translate(0px,22px)';
    }
    ...
    /* eslint-disable no-param-reassign */
    const svg = document.getElementById('barchart').getElementsByTagName('svg')[0];
    saveSvgAsPng.saveSvgAsPng(svg, `PR Open(${dateFileName}).png`);
  };
  ...
  useEffect(() => {
    ...
    if (chartData.apiCallStatus) {
      ...
      c3.generate({
        ...
        onrendered() {
          const selectorValue = document.querySelectorAll(`.prchartprocess svg .c3-axis.c3-axis-y > .tick text tspan`);
          const selectedVal = Array.from(selectorValue);
          selectedVal.forEach((element, index) => {
            if (element.innerHTML === '') {
              document.querySelector(`.prchartprocess svg .c3-axis.c3-axis-y > .tick:nth-of-type(${index + 1}) line`).classList.add('decimalValeHide');
            }
          });
        }
      });
      if (datpu.length > 24) {
        const datList = document.querySelectorAll('#barchart .c3-axis.c3-axis-x > .tick');
  ...
  return (
    <div className="openprissschart prchartprocess">
